...
const isProduction = false;//process.argv.indexOf('-p') >= 0;
...
const defaultConfigs = Array(isDevServer ? 1 : 2).fill(0).map(() => {
  return {
    cache: false,
    output: {
      path: DIST_PATH,
...
defaultConfigs[0].externals.push(function (context, request, callback) {
